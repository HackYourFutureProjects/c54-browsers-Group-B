 ...
 const observer = new MutationObserver((mutations) => {
   for (const mutation of mutations) {
     mutation.addedNodes.forEach((node) => {
       ...
       node.addEventListener('animationend', () => node.classList.remove('fade-in'), { once: true });

       // Choose gradient by screen type
       if (node.querySelector && node.querySelector(`#${START_QUIZ_BUTTON_ID}`)) {
         // Welcome
         changeBackground(-1);
       } else if (node.querySelector && node.querySelector('#restart-quiz-button')) {
         // End
         changeBackground(999);
       } else if (node.querySelector && node.querySelector(`#${NEXT_QUESTION_BUTTON_ID}`)) {
         // Question
...
export function setQuestionTheme(index) {
  ...
  const isLight = chosen.includes('#FFEB3B') || chosen.includes('#FFFDE7') || chosen.includes('#FFFFFF');
